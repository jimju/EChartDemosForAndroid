 ...
 * K线图
 */
public class KLineExamplesActivity extends BasicEChartDemoActivity {
    @Override
    public List<String> getRecyclerTitles() {
        return stringArray2List("标准K线图", "K线图");
    }
    ...
    public void onItemClick(@NonNull BaseQuickAdapter<?, ?> adapter, @NonNull View view, int position) {
        switch (position) {
            case 0:
                ...
                option.yAxis(new ValueAxis().scale(true).boundaryGap(0.01,0.01));
                option.xAxis(new CategoryAxis().axisTick(new AxisTick().onGap(false)).splitLine(new SplitLine().show(false))
                ...
                k.name("上证指数");
                k.barMaxWidth(20);
                k.data().addAll(kLineData());
                option.series(k);
                Log.d("K线图",option.toString());
                mWebView.refreshEchartsWithOption(option.toString());
                break;
            case 1:
                mWebView.setJsonFromAsset("json/kline/KLine.json");
                break;
        }
    }
